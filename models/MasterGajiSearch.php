<?php

namespace app\models;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\models\MasterGaji;

/**
 * MasterGajiSearch represents the model behind the search form of `app\models\MasterGaji`.
 */
class MasterGajiSearch extends MasterGaji
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['id', 'pegawai_id', 'bulan', 'tahun'], 'integer'],
            [['gaji_pokok', 'tunjangan_jabatan', 'tunjangan_kinerja', 'pot_bpjskes', 'pot_bpjsket'], 'number'],
            [['tanggal'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = MasterGaji::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id' => $this->id,
            'pegawai_id' => $this->pegawai_id,
            'bulan' => $this->bulan,
            'tahun' => $this->tahun,
            'gaji_pokok' => $this->gaji_pokok,
            'tunjangan_jabatan' => $this->tunjangan_jabatan,
            'tunjangan_kinerja' => $this->tunjangan_kinerja,
            'pot_bpjskes' => $this->pot_bpjskes,
            'pot_bpjsket' => $this->pot_bpjsket,
            'tanggal' => $this->tanggal,
        ]);

        return $dataProvider;
    }
}
